...
import requests
import os  # مهم حتى نجيب المفتاح من السيرفر

# ناخذ المفتاح من Render (مو من الكود)
...
async def ai_search(prompt: str = Body(...)):
    try:
        ...
        data = {
            "model": "gpt-5-mini",
            "input": f"Find the best programmer for this request: {prompt}"
        }
        ...
        # نحمي الكود حتى ما يوقع error
        reply_text = result.get("output", [{}])[0].get("content", [{}])[0].get("text", "No response")
